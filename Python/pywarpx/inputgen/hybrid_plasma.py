...
_GRID_CLASS_HYBRID = {1: "Cartesian1DGrid", 2: "Cartesian2DGrid", 3: "Cartesian3DGrid"}


def generate_picmi_hybrid_plasma(spec: HybridPlasmaSpec) -> str:
    """Generate a PICMI script for a hybrid-PIC (kinetic ions + Ohm's law electrons) run."""
    from dataclasses import asdict

    grid_class = _GRID_CLASS_HYBRID[spec.domain.dim]
    ion_mass_kg = spec.ions.mass_amu * _M_P
    ion_v_th = math.sqrt(spec.ions.temperature_eV * _Q_E / ion_mass_kg)
    ion_u_th = ion_v_th / _C
...
# Auto-generated by warpx-inputgen (hybrid_plasma PICMI).
...
# Hybrid-PIC solver: kinetic ions + Ohm's law fluid electrons
...
# Uniform ion distribution (thermal)
ions_dist = picmi.UniformDistribution(density={spec.ions.density!r}, fill_in=True)
ions = picmi.Species(
    name='ions',
    charge_state=1,
    mass={ion_mass_kg!r},
    ...
    warpx_thermal_velocity_spread=[{ion_u_th!r}, {ion_u_th!r}, {ion_u_th!r}],
)
